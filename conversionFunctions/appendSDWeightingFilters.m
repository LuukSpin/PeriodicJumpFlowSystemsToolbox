...
if ~(size(unweightedSDSystem.Bwd, 2) == size(Wwd, 1))
    error('The dimensions of the discrete-time disturbance channel of the unweighted SD system and the filter "Wwd" does not match');
end
% Check n_zd
if ~(size(unweightedSDSystem.Czd, 1) == size(Wzd, 2))
    error('The dimensions of the discrete-time performance channel of the unweighted SD system and the filter "Wzd" does not match');
end
